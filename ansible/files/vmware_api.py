...
class VmwareApi(object):
    ...
    def select_destination_host_and_datastore_by_free_space(self):
        ...
        chosen_host = chosen_datastore.host.key
...
def _get_network_device_change_spec(api, old_device, network_name=None):
...
def get_vm_config_spec(api, vm, networks):
    ...
    if len(vm_network_devices) != len(networks):
        # TODO: Better error handling
        raise Exception("Number of VM's network interfaces does not a number of provided networks")
