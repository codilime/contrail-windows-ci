...
def get_args():
    ...
    if args.vm_username and not args.vm_password:
        ...
        raise Exception('If vm-username is provided, then you have to provide vm-password as well')

    if args.vm_password and not args.vm_username:
        # TODO: Better error handling
        raise Exception('If vm-password is provided, then you have to provide vm-username as well')
...
def provision_vm(api, args):
    ...
    if not template:
        # TODO: Better error handling
        raise Exception('Source template does not exist')
    ...
    if not host or not datastore:
        # TODO: Better error handling
        raise Exception('Choosing appropriate host and datastore failed')
    ...
    config_spec = get_vm_config_spec(api, vm=template, networks=[args.mgmt_networks, args.data_network])
